...
namespace NIF
{
    public class NifRuntime
    {
        ...
        public static NifRuntime FromFile(string assetPath, out GameObject root)
        {
            var nifRuntime = new NifRuntime
            {
            ...
            var obj = new GameObject(Path.GetFileNameWithoutExtension(assetPath));
            
            nifRuntime.BuildHierarchy(nifRuntime._niFile[0] as NiNode, obj);

            nifRuntime.BuildAnimations();
        
            if (nifRuntime._skinMeshes.Count > 0)
            {
                nifRuntime.FixBones();
                foreach (var mesh in nifRuntime._skinMeshes)
                {
                    nifRuntime.BuildRig(mesh.SkinInstance, mesh.Holder);
                }
            }

            nifRuntime._niFile.Dispose();
                ...
                for (var i = 0; i < partition.VertexMap.Length; i++)
                {
                    ...
                    { 
                        ...
                        switch (j)
                        {
                            ...
                            default:
                                throw new ArgumentOutOfRangeException(
                                    $"{_assetPath} has a skinned mesh with more than 4 weights/vertex. This is not supported.");
                        }
        ...
        private string GetRelativeKeyPath(Transform source)
        {
            ...
            path = $"/{Path.GetFileNameWithoutExtension(_assetPath)}/{path}";
